...
namespace ble_client_hid {
                                          ...
                                          esp_ble_gattc_cb_param_t *param)
  {
    ...
    case ESP_GATTC_OPEN_EVT:
    {
      ...
      {
        ESP_LOGI(TAG, "[%s] Connected successfully!", this->parent()->address_str().c_str());
        break;
      }
      break;
    }
    ...
    {
      if (param->notify.conn_id != this->parent()->get_conn_id() || std::find(this->handles_registered_for_notify.begin(), this->handles_registered_for_notify.end(), param->notify.handle) == this->handles_registered_for_notify.end())
        break;
      ESP_LOGV(TAG, "[%s] ESP_GATTC_NOTIFY_EVT: handle=0x%x, value=0x%x", this->parent()->address_str().c_str(),
                param->notify.handle, param->notify.value[0]);
      this->keycode_text_sensor->publish_state(this->parse_data(param->notify.value, param->notify.value_len));
      break;
    }
  ...
  void BLEClientHID::register_keycode_text_sensor(text_sensor::TextSensor *keycode_text_sensor){
    this->keycode_text_sensor = keycode_text_sensor;
  }

  std::string BLEClientHID::parse_data(uint8_t *value, uint16_t value_len)
  {
    std::string text(value, value + value_len);
    return text;
  }

  void BLEClientHID::update()
  {
    ...
    {
      ESP_LOGW(TAG, "[%s] Cannot poll, no service or characteristic found", this->parent()->address_str().c_str());
      return;
    }
  }
