...
struct HIDReportItemValue {
  ...
  bool operator==(const HIDReportItemValue& other) const {
    return this->raw_value == other.raw_value;
  }
  HIDReportItemValue operator=(const HIDReportItemValue other) {
    return HIDReportItemValue(other.usage, other.value, other.raw_value);
  }
  std::string to_string() const;
  const HIDUsage usage;
  const int32_t value = 0;
  const int32_t raw_value = 0;
};
...
class HIDInputReportVariable : public HIDInputReportItem {
 public:
  HIDInputReportVariable(HIDUsageCollection* usage_collection, uint8_t report_count, uint8_t report_id,
                HIDLogicalRange logical_range, uint8_t report_size, uint8_t report_offset)
      : HIDInputReportItem(usage_collection, report_count, report_id, logical_range, report_size, report_offset) {
